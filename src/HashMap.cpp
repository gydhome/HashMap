...
unsigned long File::hash(char *s)
{
	...
	for(slength = strlen(s); isspace(s[slength - 1]); slength--)
	{
		for(pack = 0, j = 0; ; j++, i++)
		{
			pack |= (unsigned long)s[i]; // include s[i] in the rightmost byte of pack
			if(j == 3 || i == slength - 1)
			{
				i++; break;
			}
			pack <<= 8;      // xor at one time 8 bytes from s;
		}
		nxor ^= pack;   // last iteration may put less than 8 bytes in pack
	}
	return (nxor % tableSize) * bucketSize * recordLen;
...
// data from overflow file and outfile are all stored o=in outfile and
...
void File::combineFiles()
{
	...
	{
		if(rec[0] != delMarker){      // transfer from overflow to outfile only
			counter++;
			outfile << rec << setw(strlen(rec) - recordLen);
		}
	}
	// load to array pointers positions of valid data stored om output file;
